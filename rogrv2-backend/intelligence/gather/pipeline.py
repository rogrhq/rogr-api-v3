...
from intelligence.analyze.quality import quality_for
...
def build_evidence_for_claim(claim_text: str, candidates: List[Dict]) -> List[Dict]:
    ...
    Output: evidence list [{source:{...}, stance, confidence, quality_letter, features}]
    """
    sources = normalize_candidates(candidates)
    evidence: List[Dict] = []
    for s in sources:
        stance, conf = stance_for(claim_text, s)
        q_letter, feats = quality_for(s)
        evidence.append({
            "source": s,
            "stance": stance,
            "confidence": round(float(conf), 3),
            "quality_letter": q_letter,
            "features": feats,
        })
    return evidence