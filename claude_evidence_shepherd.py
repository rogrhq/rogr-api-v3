...
class ClaudeEvidenceShepherd(EvidenceShepherd):
    """Claude-powered evidence shepherd for smart fact-checking with superior context handling"""
...
Return ONLY JSON:
{
  ...
  "reasoning": "Brief strategy explanation"
        ...
        try:
            ...
            print(f"Error parsing Claude strategy response: {e}")
            return self._fallback_strategy(claim_text)
    ...
    def filter_evidence_batch(self, claim_text: str, evidence_batch: List[EvidenceCandidate]) -> List[ProcessedEvidence]:
        ...
        # Try batch processing with Claude's superior context handling
        try:
            batch_results = self._batch_score_evidence_claude(claim_text, evidence_to_process)
            if batch_results:
                return batch_results
        except Exception as e:
            print(f"Claude batch processing failed: {e}, falling back to individual scoring")
        ...
        for i, ev in enumerate(processed_evidence[:3]):  # Show first 3 for debugging
            print(f"  Claude Evidence {i+1}: score={ev.ai_relevance_score}, confidence={ev.ai_confidence}")
...
CRITICAL: key_excerpt must be under 100 characters with escaped quotes (\")"""
        ...
        try:
            ...
            print(f"Error parsing Claude relevance response: {e}")
            return self._fallback_evidence_score(claim_text, evidence)
